...
ParagraphPart = ParagraphText | Block | LatexBlock | List


@dataclass
class Paragraph:
    """A sequence of text runs, LaTeX blocks, blocks, and lists.
    ...
    elements: list[ParagraphPart] = field(default_factory=list)
    ...
    def try_parse(cls, cursor: Cursor[FirstPassElement]) -> Paragraph | None:
        ...
        while (element := cursor.peek()) is not None:
            ...
            if text is not None:
                ...
                continue
            if isinstance(element, (Block, LatexBlock, List)):
...
__all__ = ["Paragraph", "ParagraphPart"]
